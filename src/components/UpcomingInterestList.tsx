...
export const UpcomingInterestList: React.FC<UpcomingInterestListProps> = ({
  ...
  const filteredLoans = activeLoans.filter((loan) => {
    ...
    const matchesFilter = 
      filterType === 'all' ||
      (filterType === 'overdue' && badge.status === 'overdue') ||
      (filterType === 'today' && badge.status === 'today') ||
      (filterType === 'upcoming' && badge.status !== 'overdue' && badge.status !== 'today');
    
    const matchesSearch = 
      loan.borrowerName.toLowerCase().includes(searchQuery.toLowerCase()) ||
      (loan.borrowerPhone && loan.borrowerPhone.includes(searchQuery));

    return matchesFilter && matchesSearch;
  });

  const overdueCount = activeLoans.filter((l) => formatDueDateBadge(l.nextInterestDueDate).status === 'overdue').length;
  const todayCount = activeLoans.filter((l) => formatDueDateBadge(l.nextInterestDueDate).status === 'today').length;
    ...
    <div className="space-y-3.5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-1">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">{filteredLoans.length} of {activeLoans.length} active borrowers</p>
          </div>
        </div>

        {/* Filter Pills for Tablet & Touch Screens */}
        <div className="flex items-center gap-1.5 bg-slate-900/90 border border-slate-800 p-1 rounded-xl self-start sm:self-auto overflow-x-auto max-w-full">
          <button
          ...
          >
            <span>Overdue</span>
            {overdueCount > 0 && (
              <span className="bg-rose-950/80 text-rose-300 text-[10px] px-1.5 py-0.2 rounded-full font-bold">
                {overdueCount}
              </span>
            )}
          </button>
      ...
      {filteredLoans.length === 0 ? (
        <div className="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-8 text-center">
          <p className="text-xs text-slate-400">No loan entries match the selected filter.</p>
          <button
            onClick={() => setFilterType('all')}
              ...
              >
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-11 h-11 rounded-2xl bg-gradient-to-tr ${loan.borrowerAvatarColor} flex items-center justify-center text-white font-bold text-base shadow-md shrink-0`}
                    >
                      {loan.borrowerName
                        .split(' ')
                        .map((n) => n[0])
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs font-semibold text-slate-300 capitalize">
                      {loan.recurrence === 'custom_days'
                        ...
                        : loan.dueDayOfMonth
                        ? `Monthly (Day ${loan.dueDayOfMonth})`
                        : loan.recurrence}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between gap-2 pt-1 border-t border-slate-800/60">
                  <div className="flex items-center gap-1.5">
                    ...
                    <a
                      ...
                      className="p-2.5 rounded-xl bg-slate-800 text-sky-400 hover:bg-sky-500/20 hover:text-sky-300 transition-colors flex items-center gap-1 text-xs font-medium active:scale-95"
                    >
                    ...
                    <button
                      ...
                      className="p-2.5 rounded-xl bg-slate-800 text-slate-300 hover:bg-emerald-500/20 hover:text-emerald-300 transition-colors active:scale-95"
                    >
                    ...
                    <button
                      ...
                      className="p-2.5 rounded-xl bg-slate-800 text-slate-300 hover:bg-slate-700 hover:text-white transition-colors relative active:scale-95"
                    >
                  ...
                  <div className="flex items-center gap-1.5 ml-auto">
                    <button
                      ...
                      className="flex items-center gap-1 bg-purple-950/40 hover:bg-purple-900/50 text-purple-300 border border-purple-800/60 text-xs font-semibold px-3 py-2.5 rounded-xl transition-all active:scale-95"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold px-3.5 py-2.5 rounded-xl shadow-md shadow-emerald-700/30 transition-all active:scale-95"
                    >
